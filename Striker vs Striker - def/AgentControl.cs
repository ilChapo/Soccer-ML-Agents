...
public class AgentControl : Agent
{
    ...
    public GameObject ownGoalArea;
    public GameObject raycastFrontal;
    [HideInInspector]
    public Team team;
    [HideInInspector]
    public Vector3 goalAreaPos;
    [HideInInspector]
    public Vector3 ownGoalAreaPos;
    [HideInInspector]
    ...
    public override void Initialize() {
      ...
      raycastFrontal = gameObject.transform.GetChild(6).gameObject;
      var c = raycastFrontal.GetComponent<RayPerceptionSensorComponent3D>();
      spec = c.GetRayPerceptionInput();

      playerBody = gameObject.GetComponent<Rigidbody>();
      m_ballRb = ball.GetComponent<Rigidbody>();
      goalAreaPos = goalArea.GetComponent<Transform>().position;
      ownGoalAreaPos = ownGoalArea.GetComponent<Transform>().position;
     ...
     public override void CollectObservations(VectorSensor sensor)
     {
         ...
         // sensor.AddObservation((float)m_BehaviorParameters.TeamId);
         // sensor.AddObservation(transform.position);
         // sensor.AddObservation(goalAreaPos);
         // sensor.AddObservation(ownGoalAreaPos);
         //sensor.AddObservation(m_ballRb.position);
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers) {
       ...
       obs = RayPerceptionSensor.Perceive(spec);

       // if(!BallSeen(obs)) {
       //   Debug.Log(" BLAUNO ESTÀ VEIENT LA BOLA SISISIS");
       //   AddReward(-0.001f);
       // }
    ...
    private bool BallSeen (RayPerceptionOutput obs) {

      foreach (var rayInfo in obs.RayOutputs)
               {
                   if (rayInfo.HitTagIndex == 2) {
                     return true;
                   } //bola
               }
